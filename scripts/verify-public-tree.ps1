...
try {
    ...
    Invoke-Native "readme install order" { python -c "from pathlib import Path; text=Path('README.md').read_text(encoding='utf-8'); public='From the public GitHub repository:'; local='From a local checkout:'; required=(public, local, 'KEYSURGEON_POST_PUBLISH_READY', 'https://github.com/Nosafune/keysurgeon', 'https://nosafune.github.io/keysurgeon/', 'https://github.com/Nosafune/keysurgeon/releases/tag/v0.2.0', 'git+https://github.com/nosafune/keysurgeon.git'); missing=[x for x in required if x not in text]; assert not missing, missing; assert text.index(public) < text.index(local); print('README_INSTALL_ORDER_OK')" }
    ...
    Invoke-Native "readme publish status" { python -c "from pathlib import Path; text=Path('README.md').read_text(encoding='utf-8'); required=('What You Get In 30 Seconds','browser keyboard tester says a key works','Current Publish Status','KeySurgeon is published publicly','https://github.com/Nosafune/keysurgeon','https://nosafune.github.io/keysurgeon/','https://github.com/Nosafune/keysurgeon/releases/tag/v0.2.0','keysurgeon-v0.2.0-windows-x64.exe','KEYSURGEON_POST_PUBLISH_READY','hardware-smoke-pass','post-publish-audit.ps1','launch-readiness.ps1'); forbidden=(('public-ready locally, not ' + 'published'),('GitHub repository and origin remote do not ' + 'exist yet'),('remote selftest and Pages workflow runs do not ' + 'exist yet'),('no GitHub release asset is ' + 'published yet')); missing=[x for x in required if x not in text]; present_forbidden=[x for x in forbidden if x in text]; assert not missing, missing; assert not present_forbidden, present_forbidden; assert text.index('What You Get In 30 Seconds') < text.index('Current Publish Status'); print('README_PUBLISH_STATUS_OK', len(required))" }
